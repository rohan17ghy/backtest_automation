...
export class BrowserFactory {
    ...
    private static async initBrowser(): Promise<Browser> {
        ...
        console.log(
            `Connecting to browser with URL ${process.env.BROWSER_WEB_SOCKET_DEBUGGER_URL}`
        );
...
export class GoCharting {
    ...
    public async navigateToPageWithDate(
        page: Page,
        symbol: Symbol,
        dateTime: Date
    ) {
    ...
    public async applyTheme(page: Page) {
        ...
        if (process.env.DARK_MODE == "true") {
            ...
            const bgColorPickerSelector =
                ".sketch-picker > .flexbox-fix:nth-child(3) > div:first-child > div:first-child > input";
            ...
            const opacitySelector =
                ".sketch-picker > .flexbox-fix:nth-child(3) > div:last-child > div:first-child > input";
    ...
    public async closeAdIfPresent(page: Page) {
        ...
        //Type 1 ad
        if (
            await waitForSelectorWithBoolean(page, ".css-nnw530-CloseContainer")
        ) {
        ...
        //Type 2 ad
        if (
            await waitForSelectorWithBoolean(page, ".css-juko39-CloseContainer")
        ) {
    ...
    public async goToDateTime(page: Page, dateTime: Date) {
        ...
        console.log(
            `Navigating to ${date}/${month}/${year} ${hour}:${minute}, DateTime: ${dateTime}`
        );
    ...
    public async scrapeGoToDateTime(page: Page, dateTime: Date) {
        ...
        console.log(
            `Navigating to ${date}/${month}/${year} ${hour}:${minute}${ampm}, DateTime: ${dateTime}`
        );
...
export async function displayNearITMBNStrikes(symbol: Symbol, dateTime: Date) {
    ...
    await goCharting.displayStrikes(
        goCharting.ceBrowser,
        strikes.ceStrikes as string[],
        dateTime
    );
    await goCharting.displayStrikes(
        goCharting.peBrowser,
        strikes.peStrikes as string[],
        dateTime
    );
}
